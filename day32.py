...
def findHighestJoltageCombi(bank):
    joltageCombi = 0
    for leftElement in range(0,len(bank)-11):
        for rightElement in range(leftElement+13,len(bank)+1):
            if leftElement != rightElement-1:
                print("LeftElem: "+ bank[leftElement:leftElement+12] + " RightElem: "+ bank[rightElement-12:rightElement])
                leftElem = int(bank[leftElement:leftElement+12])
                rightElem = int(bank[rightElement-12:rightElement])
                if (leftElem > joltageCombi): joltageCombi = leftElem
    return joltageCombi
